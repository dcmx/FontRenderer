...
Line::Line(const std::string line) :
		line(line), offset(0.0, 0.0), height(0), bezierDegree(2) {
}
...
void Line::output(std::ostream& out) const {
	...
	for (std::list<std::list<const Primitive*> >::const_iterator it1 = primitives.begin(); it1 != primitives.end();
			++it1) {
...
void Line::render(FT_Face &face, const double posY, const double spacing) {
	...
	for (std::string::const_iterator it = line.begin(); it != line.end(); ++it) {
		...
		if (FT_Load_Glyph(face, gindex, FT_LOAD_NO_BITMAP))
			throw std::string("Could not load glyph for character ").append(1, ch);
